...
exec  1> $"/user/logfile.txt"
exec  2> $"/user/errors"
#3>&1 4>&2 >
START=$(date +%s.%2N)
#Branch 1
if [ "$output" = "" ]; then
	for inputs in /user/inputFile*
	do
		$compiler /user/$file -< "$inputs"
		echo " "
	done
...
#Branch 2
	...
	#In case of compile errors, redirect them to a file
        $compiler /user/$file $addtionalArg #&> /user/errors.txt
	#Branch 2a
	if [ $? -eq 0 ];	then
		for inputs in /user/inputFile*
		do
...
#head -100 /user/logfile.txt
#touch /user/completed
END=$(date +%s.%2N)
...
mv /user/logfile.txt /user/completed
chmod 777 /user/completed